...
Xuint8 carrier_hdmi_out_config[CARRIER_HDMI_OUT_CONFIG_LEN][3] =
{
		...
		//    R0x16[  7] = Output Video Format = 0 (444) //change to 422 -1
		...
		{ADV7511_ADDR>>1, 0x18, 0xAC},	//AC -> 2C
...
//	vgen_config( &(pDemo->vtc_hdmio_generator), pDemo->hdmio_resolution, 1 );
	...
	Xuint8 connected = 0;
	while (!connected){
		pDemo->hdmi_out_iic.fpIicRead(&(pDemo->hdmi_out_iic), ADV7511_ADDR>>1,  0x42, &connected, 1 );
		connected &= 0x60;
		xil_printf("Monitor status %d\n\r", connected);
	}
	xil_printf("Detected Monitor\n\r");
	while (1)
	{
		pDemo->hdmi_out_iic.fpIicRead(&(pDemo->hdmi_out_iic), ADV7511_ADDR>>1,  0x42, &connected, 1 );
		connected &= 0x60;
		xil_printf("Monitor status %d\n\r", connected);
		if ( iterations > 0  && connected)
		{
